...
#define DEAD_PIN_4 4
#define THROTTLE_A1_PIN 5
#define THROTTLE_A2_PIN 6
...
void controlMotor() {
    ...
    if (throttle >= 122 && throttle <= 132) {
        ...
        analogWrite(THROTTLE_A2_PIN, 0);
        Serial.println("--------------------------------------- Max backward speed");
        ...
        Serial.println("--------------------------------------- Max forward speed");
...
int servoJitterFilter(int angle) {
  if (angle >= 92 && angle <= 99) { // Remove center jitter
    angle = 95;
  } else if (angle < 33) { // Remove lower jitter
    angle = 30;
  } else if (angle > 157) { // Remove upper jitter
    angle = 160;
  }
  return angle;
}