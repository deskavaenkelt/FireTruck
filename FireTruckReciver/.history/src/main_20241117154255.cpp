...
void controlServos()
{
    ...
    // New steering motor control
    if (angle >= 92 && angle <= 99) {  // Center position
        analogWrite(STEERING_A3_PIN, 0);
        analogWrite(STEERING_A4_PIN, 0);
    }
    else if (angle < 92) {  // Turn left
        analogWrite(STEERING_A3_PIN, map(angle, 92, SERVO_MIN_RANGE, 0, MAX_MOTOR_SPEED));
        analogWrite(STEERING_A4_PIN, 0);
    }
    else if (angle > 99) {  // Turn right
        analogWrite(STEERING_A3_PIN, 0);
        analogWrite(STEERING_A4_PIN, map(angle, 99, SERVO_MAX_RANGE, 0, MAX_MOTOR_SPEED));
    }
}
