...
void controlServos()
{
    ...
    if (angle >= 124 && angle <= 130) {  // Center deadzone
        ...
        targetOutput = -MAX_ANGLE;
    }
    else if (angle < 124) {  // Left turn with mapping
        targetOutput = -map(angle, 20, 124, MAX_ANGLE, 0);
    }
    else if (angle > 235) {  // Max right
        targetOutput = MAX_ANGLE;
    }
    else if (angle > 130) {  // Right turn with mapping
        targetOutput = map(angle, 130, 235, 0, MAX_ANGLE);
    }

    // Smooth transition
    int smoothedOutput = lastSteeringOutput + (targetOutput - lastSteeringOutput) * SMOOTHING_FACTOR;
    lastSteeringOutput = smoothedOutput;

    // Apply smoothed output to motors
    if (smoothedOutput > 0) {  // Right turn
        analogWrite(STEERING_A3_PIN, abs(smoothedOutput));
        analogWrite(STEERING_A4_PIN, 0);
    } else {  // Left turn or center
        analogWrite(STEERING_A3_PIN, 0);
        analogWrite(STEERING_A4_PIN, abs(smoothedOutput));
    }

    Serial.print("Target: ");
    Serial.print(targetOutput);
