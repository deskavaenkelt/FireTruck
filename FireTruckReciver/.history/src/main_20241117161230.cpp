...
void controlServos()
{
    ...
    if (angle >= 124 && angle <= 130) {  // Center deadzone
        ...
        analogWrite(STEERING_A3_PIN, 0);
        analogWrite(STEERING_A4_PIN, MAX_MOTOR_SPEED);
    }
    else if (angle < 124) {  // Left turn with mapping
        analogWrite(STEERING_A3_PIN, 0);
        analogWrite(STEERING_A4_PIN, map(angle, 20, 124, MAX_MOTOR_SPEED, 0));
    }
    else if (angle > 235) {  // Max right
        analogWrite(STEERING_A3_PIN, MAX_MOTOR_SPEED);
        analogWrite(STEERING_A4_PIN, 0);
    }
    else if (angle > 130) {  // Right turn with mapping
        analogWrite(STEERING_A3_PIN, map(angle, 130, 235, 0, MAX_MOTOR_SPEED));
        analogWrite(STEERING_A4_PIN, 0);
    }
}
