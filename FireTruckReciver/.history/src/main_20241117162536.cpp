...
void controlServos()
{
    ...
    // Wider deadzone for stability
    if (angle >= 120 && angle <= 134) {  // Center deadzone
        analogWrite(STEERING_A3_PIN, 0);
        analogWrite(STEERING_A4_PIN, 0);
    }
    // Quick initial pulse then reduced power for holding position
    else if (angle < 20) {  // Max left
        analogWrite(STEERING_A3_PIN, 0);
        analogWrite(STEERING_A4_PIN, MAX_ANGLE);
        delay(50);  // Short pulse at full power
        analogWrite(STEERING_A4_PIN, MAX_ANGLE * 0.7);  // Reduced power to hold
    }
    else if (angle > 235) {  // Max right
        analogWrite(STEERING_A3_PIN, MAX_ANGLE);
        analogWrite(STEERING_A4_PIN, 0);
        delay(50);  // Short pulse at full power
        analogWrite(STEERING_A3_PIN, MAX_ANGLE * 0.7);  // Reduced power to hold
    }
    // Progressive control for minor adjustments
